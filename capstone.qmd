...
![With the line of best fit (blue) overlaid on the actual historic values for stream temperatures (green), this graph displays the predictions (pink) into the future for stream temperature, given all values are identical to 2024 aside from the weather temperature, which is steadily increasing at a slope of 2.5 degrees Celsius of warming by 2050. While this pseudo-future data is not highly accurate, the predictions follow the expected sinusoidal pattern.](projects/capstone/stream_predictions.png){fig-alt="Best Performing Stream Temperature Predictive Model with Future Estimates" width="90%"}
